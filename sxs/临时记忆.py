...
model = ChatTongyi(model="qwen3-max",api_key=get_openai_key())
...
def print_prompt(full_prompt):#输出提示词内容，会被自动转换
    print("="*20, full_prompt.to_string(), "="*20)#重载了，只提取出message部分
    return full_prompt
...
#session_config--- get_history---store---prompt---model

if __name__ == '__main__':
    # 固定格式，添加LangChain的配置，为当前程序配置所属的session_id
    session_config = {
        "configurable": {
            "session_id": "user_001"#只配置会话id
        }
    }
